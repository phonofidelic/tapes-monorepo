...
/**
 * What a panel is allowed to believe about who is connected.
 *
 * The list itself is simple. The states around it are the ones that mislead. A
...
describe('useConnectedDevices', () => {
  ...
  // The subscription must be in place before the snapshot is requested. A
  // change landing in between is otherwise lost.
  ...
  // A channel that answers with nothing at all.
  ...
  // An event that lands first is accurate. A failing snapshot arriving after
  // it must not wipe a list we know is real.
  ...
  // Only the desktop app hosts. A guest has no registry to read, so it must
  // not be shown an empty guest list of its own.
